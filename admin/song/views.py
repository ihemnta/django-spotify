...
def save(request):
    if request.method == 'POST':
        ...
        mood = Mood.objects.filter(pk=mood)
        if not mood:
        ...
        artist = Artist.objects.filter(pk=artist)
        if not artist:
        ...
        songObj = Song(song_name=name, song_des=description, song_length=length, genre=genre, mood=mood, artist=artist, song_file=song_file)
...
def delete(request, id):
    ...
    if request.method == 'GET':
        ...
        if not song:
            messages.error(request, "No such sound found!")
            return redirect('admin.song.index')


        else:
            #for deleting the song file
            song_obj = song.get()
            song_obj.song_file.delete()


            #for deleting the record from the database
            song.delete()
...
def edit(request, id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'adminTemplates/song/edit.html', {'genre':genre, 'mood':mood, 'artist':artist, 'song': song})
